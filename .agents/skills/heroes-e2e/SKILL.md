...
```bash
.Codex/skills/heroes-e2e/scripts/hoc-e2e.sh match       # human-vs-human: full pick/ban -> play match, print TWO links
.Codex/skills/heroes-e2e/scripts/hoc-e2e.sh vs-ai       # human-vs-AI: ONE link, opens in pick phase against the bot
.Codex/skills/heroes-e2e/scripts/hoc-e2e.sh placement   # SKIP pick/ban: start in placement, randomized rosters
.Codex/skills/heroes-e2e/scripts/hoc-e2e.sh status      # what's running (incl. monitor)
.Codex/skills/heroes-e2e/scripts/hoc-e2e.sh monitor     # start watchdog (if needed) + print recorded anomalies
.Codex/skills/heroes-e2e/scripts/hoc-e2e.sh cleanup     # stop server + client + monitor (leaves DB containers)
```
