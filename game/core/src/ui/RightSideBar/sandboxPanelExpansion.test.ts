...
describe("sandbox right-sidebar panel expansion", () => {
    test("starts with both setup tools visible", () => {
    ...
    test("allows a one-pixel layout tolerance", () => {
        expect(sandboxSidebarOverflowsVertically({ clientHeight: 800, scrollHeight: 801 })).toBe(false);
        expect(sandboxSidebarOverflowsVertically({ clientHeight: 800, scrollHeight: 802 })).toBe(true);
