...
export interface LeftSidebarPortraitArt {
    source?: string;
    /** False when the sidebar source owns a standalone canvas and must not inherit pick-card coordinates. */
    usesFraming?: boolean;
...
/** The complete test-server `*_left_screen_x2.webp` portrait set. */
...
export const resolveLeftSidebarPortraitArt = (creatureId: number): LeftSidebarPortraitArt => {
    const source = LEFT_SIDEBAR_PORTRAIT_ART_CHECKPOINT_X[creatureId];
    return source ? { source, usesFraming: false, fit: "contain", baseScale: 1 } : {};
};
