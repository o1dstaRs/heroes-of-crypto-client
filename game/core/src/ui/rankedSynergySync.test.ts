...
describe("ranked synergy snapshot sync", () => {
    ...
    test("preserves both locally-derived faction bonuses across placement snapshots for the same game", () => {
        const store = createStore(["Life:1:1", "Life:2:1"], []);

        const currentGameId = syncRankedSnapshotSynergies(store, placementSnapshot("game-b"), "game-b");

        expect(currentGameId).toBe("game-b");
        expect(store.values.get(TeamVals.LOWER)).toEqual(["Life:1:1", "Life:2:1"]);
        expect(store.calls).toHaveLength(0);
    });

    test("replaces both teams with authoritative synergies once the fight starts", () => {
        const store = createStore(["Life:1:1", "Life:2:1"], ["Nature:1:1", "Nature:2:1"]);
        ...
        const currentGameId = syncRankedSnapshotSynergies(
            store,
            {
                gameId: "game-b",
                fightStarted: true,
                lowerSynergies: ["Might:1:3", "Might:2:3"],
                upperSynergies: ["Chaos:1:2", "Chaos:2:2"],
            },
            "game-b",
        );

        expect(currentGameId).toBe("game-b");
        expect(store.values.get(TeamVals.LOWER)).toEqual(["Might:1:3", "Might:2:3"]);
        expect(store.values.get(TeamVals.UPPER)).toEqual(["Chaos:1:2", "Chaos:2:2"]);
        expect(store.calls).toHaveLength(2);
    });
});
