import { RenderableUnit as LegacyRenderableUnit } from "./RenderableUnit";
import {
    Container,
    Sprite,
    Graphics,
...
export function creatureWalkAnimationEnabledForUnit(unitName: string, labPreview = false): boolean {
    return (
        CREATURE_SPRITE_ANIMATION_SETTINGS.enabled ||
        usesApprovedBaseAnimations(unitName) ||
        (labPreview && (unitName === "Fairy" || unitName === DRYAD_UNIT_NAME)) ||
        (labPreview &&
        ...
        (usesApprovedBaseAnimations(unitName) &&
            (["hit", "death"].includes(stateName) ||
                (isOrcRangedAttack(stateName) &&
                    ["Scavenger", "Orc", "Arbalester", "Dryad", "Centaur"].includes(unitName)) ||
                (isOrcMeleeAttack(stateName) &&
                    ["Fairy", "Leprechaun", "Berserker", "Orc", "Arbalester", "Dryad", "Centaur"].includes(
                        unitName,
                    )))) ||
...
export class RenderableUnit extends Unit {
    public static override [Symbol.hasInstance](value: unknown): boolean {
        return (
            typeof value === "object" &&
            value !== null &&
            (RenderableUnit.prototype.isPrototypeOf(value) || LegacyRenderableUnit.prototype.isPrototypeOf(value))
        );
    }
    ...
    public static fromBase(base: Unit, texResolver: TexResolver): RenderableUnit {
        if (!usesApprovedBaseAnimations(base.getName()))
            return LegacyRenderableUnit.fromBase(base, texResolver) as unknown as RenderableUnit;
    ...
    /** Clear the retired hover/selection ground effect. */
    private updateActiveAura(_worldRoot: Container, _gs: GridSettings, _pos: HoCMath.XY, _nowMs: number): void {
        // Hover/selection must not add a ground halo around the creature.
        // Hide any existing effect too, including after a live scene refresh.
        if (this.activeAura) this.activeAura.visible = false;
        if (this.activeTurnFireSprite) this.activeTurnFireSprite.visible = false;
    }
    ...
    public setCreatureAnimationLabPreviewEnabled(enabled: boolean): void {
        ...
        if (labChanged && this.getName() === DRYAD_UNIT_NAME) {
        ...
        if (this.getName() === CENTAUR_UNIT_NAME || this.getName() === "Fairy") {
            if (!labChanged) return;
            const previousHeight = this.sprite ? this.sprite.texture.height * this.sprite.scale.y : undefined;
            this.stopSelectionAnimationInternal();
            this.selectionAnimationStartedAtMs = performance.now();
    ...
    public startBoardWalkAnimation(
        ...
        if (!creatureWalkAnimationEnabledForUnit(props.name, this.creatureAnimationLabPreviewEnabled)) {
            ...
            this.restoreWidthAfterArbalesterWalk();
        ...
        const config: UnitAtlasConfig | null = fairyLabWalk
            ? {
     ...
     * has the user-requested 30% faster cadence.
    ...
    public setBoardWalkDistanceCells(distanceCells: number): void {
        ...
        const unitName = this.getUnitProperties().name;
        const cadenceSpeed = unitName === "Fairy" && this.creatureAnimationLabPreviewEnabled ? FAIRY_LAB_WALK_SPEED : 1;
    ...
    private syncCreaturePalette(now = performance.now()): void {
    ...
    private restoreWidthAfterArbalesterWalk(): void {
    ...
    public finishBoardWalkAnimationAfterFullCycle(onLandingComplete?: () => void): boolean {
        ...
        const startOutro = (): boolean => {
            ...
            if (this.creatureAnimationLabPreviewEnabled && this.getName() === "Fairy") this.syncCreaturePalette();
            return true;
        };
    ...
    public stepSpawnAnimation(dt: number): void {
        ...
        const fairyLabTransitionTimeScale =
            this.creatureAnimationLabPreviewEnabled && this.getName() === "Fairy" ? 4 * FAIRY_LAB_TRANSITION_SPEED : 1;
        this.stepBoardWalkAnimation(dt * 1000 * fairyLabTransitionTimeScale);
        // --- One Shot animation ---
    ...
    public playDryadRangedShot(state: string, shot: AbortController, onRelease: () => void): boolean {
        ...
        this.dryadRangedShot = shot;
        this.oneShotAnim.projectileReleaseFrame = 3;
    ...
    public getDryadArrowLength(): number | undefined {
        if (!this.sprite || !this.oneShotAnim || this.getName() !== DRYAD_UNIT_NAME) return undefined;
        const meta = animationAtlases["Dryad Lab"]?.[this.oneShotAnim.stateName];
    ...
    public getRangedProjectileOrigin(target: HoCMath.XY, gs: GridSettings): HoCMath.XY {
        ...
        if (this.creatureAnimationLabPreviewEnabled && this.getName() === DRYAD_UNIT_NAME && this.oneShotAnim) {
            const origin = animationAtlases["Dryad Lab"]?.[this.oneShotAnim.stateName]?.projectileOrigin as
    ...
    public playOneShotAnimation(stateName: string, onComplete?: () => void, forcePreview = false): boolean {
        ...
        if ((fairyLabConfig || centaurLabConfig || dryadLabConfig || leprechaunLabConfig) && this.walkAnim)
            this.stopBoardWalkAnimation();
        const previousArbalesterWalkScale =
