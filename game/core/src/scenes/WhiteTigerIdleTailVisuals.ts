...
import { WHITE_TIGER_TAIL_GLSL, WHITE_TIGER_TAIL_SPEED } from "./WhiteTigerIdleTail";
...
class WhiteTigerTailFilter extends Filter {
    ...
    public update(nowMs: number): void {
        this.resources.tail.uniforms.uPhase = (nowMs * 0.001 * WHITE_TIGER_TAIL_SPEED) % (Math.PI * 2);
    }
    public override apply(...args: Parameters<Filter["apply"]>): void {
        const uniforms = this.resources.tail.uniforms;
...
const filters = new WeakMap<Sprite, WhiteTigerTailFilter>();

/** Keep tail phase continuous through atlas holds and loop boundaries. */
export function syncWhiteTigerIdleTail(sprite: Sprite, enabled: boolean, nowMs: number): void {
    let filter = filters.get(sprite);
    if (enabled && !filter) {
        filter = new WhiteTigerTailFilter(sprite);
