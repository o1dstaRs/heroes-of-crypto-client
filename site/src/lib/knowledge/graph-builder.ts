...
function formulaSpecs(): FormulaSpec[] {
    ...
    return [
        ...
        },
        {
            ...
            text: bullet([
                `Each army has ${minutes} minutes per lap. A turn lasts min(${maxTurn} s, remaining budget ÷ stacks still to act), always leaving ${minTurn} s for each of them — an army that fielded 8 stacks gets about 30 s per turn at the start of a lap, one that fielded 4 or fewer ${maxTurn} s. Time spent before an Hourglass wait isn't charged.`,
            ...
            textRu: bullet([
                `У каждой армии ${minutes} минуты на круг. Ход длится min(${maxTurn} с, остаток бюджета ÷ стеки, которые ещё не ходили), и каждому из них всегда остаётся ${minTurn} с — армия, выставившая 8 стеков, в начале круга получает около 30 с на ход, выставившая 4 и меньше — ${maxTurn} с. Время до ожидания через Hourglass не списывается.`,
