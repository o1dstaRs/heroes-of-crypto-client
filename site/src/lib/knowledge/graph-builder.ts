...
function augmentSpecs(): AugmentSpec[] {
    ...
    return [
        ...
        },
        {
            ...
            note: "For armies that intend to fight up close. The bonus follows the attack a unit has selected (set on its own turn), not the blow: a shooter on its ranged selection retaliates in melee with the Sniper augment's bonus rather than this one. It also raises what is priced from the attack, such as the Black Dragon's Fire Breath.",
            noteRu: "Для армий, которые собираются драться вплотную. Бонус следует за выбранной атакой юнита (она выбирается в его ход), а не за ударом: стрелок с выбранной дальней атакой отвечает в ближнем бою с бонусом «Стрельбы», а не этим. Он повышает и то, что считается от атаки, например Fire Breath у Black Dragon.",
        ...
        },
        {
            ...
            summary:
                "Team-wide magic damage bonus: offensive spells, Fire Wall, Chain Lightning, Fire Breath and Fire Shield. Fireforged Sword is not raised.",
            summaryRu:
                "Командный бонус к магическому урону: атакующие заклинания, Fire Wall, Chain Lightning, Fire Breath и Fire Shield. Fireforged Sword не усиливается.",
...
const supplyExamples = (): string => {
    ...
    // Level 3 is an all-Life army, so the examples are Life creatures only.
    return ["Peasant", "Arbalester", "Healer", "Monk", "Angel"]
...
const synergySpecs: SynergySpec[] = [
    {
        ...
        detail: `Applied once, when the fight starts: every stack's size is multiplied and rounded down, so small stacks gain nothing — at level 1 the first extra creature needs 17 in the stack, at level 2 nine, at level 3 six — and level-4 stacks of 1–3 creatures never grow. Creatures summoned later get nothing. At level 3: ${supplyExamples()}. It is worth most to armies of big low-level stacks. Level 3 takes six different Life creatures — an all-Life army — and, when its level-3 pick is the Monk, moves stack power for everyone: its Monk stack grows past 1,250 experience (8 → 9, 1,278) and becomes the board's strongest, so while all nine live, every enemy stack worth 1,022 experience or less and your own level-4 stacks of 1–3 creatures (they don't grow) sit at stack power 4 — a fifth off their stack-powered abilities, and no Meteorite, Meteor Shower or Wind Flow, which need 5. One Monk's death (1,136) ends it. Without the Monk the biggest grown stack is a Pikeman stack at about 1,220 experience, which leaves every stack of 1,000 or more at stack power 5.`,
        detailRu: `Применяется один раз, в начале боя: размер каждого стека умножается и округляется вниз, поэтому маленькие стеки ничего не получают — на уровне 1 первое лишнее существо появляется при 17 в стеке, на уровне 2 — при 9, на уровне 3 — при 6, — а стеки 4 уровня из 1–3 существ не растут никогда. Призванные позже существа ничего не получают. На уровне 3: ${supplyExamples()}. Больше всего она даёт армиям из больших стеков низкого уровня. 3 уровень требует шести разных существ Жизни — армии целиком из Жизни — и, если её пик 3 уровня — Monk, меняет силу стека для всех: её стек Monk вырастает больше 1250 опыта (8 → 9, 1278) и становится сильнейшим на поле, поэтому пока живы все девять, каждый вражеский стек на 1022 опыта и меньше и ваши стеки 4 уровня из 1–3 существ (они не растут) держатся на силе 4 — способности от силы стека слабее на пятую часть, а Meteorite, Meteor Shower и Wind Flow, требующие 5, недоступны. Гибель одного Monk (1136) это прекращает. Без Monk самый большой выросший стек — Pikeman, около 1220 опыта, и все стеки от 1000 опыта остаются на силе 5.`,
    ...
    return [
        ...
        },
        {
            ...
            text: [
                ...
                "While a stack is on Morale or Dismorale — or under Courage (+20), Sadness (−20) or both at once (0) — its morale cannot change; on Morale its buffs don't count down that lap, on Dismorale its debuffs and effects don't. Madness and Mechanism units sit at 0 and never roll (unless Broken, when those abilities are off).",
                "Exact changes:",
                bullet([
            ...
            textRu: [
                ...
                "Пока стек под Morale или Dismorale — или под Courage (+20), Sadness (−20) или обоими сразу (0), — его мораль не меняется; под Morale его баффы в этом круге не убывают, под Dismorale не убывают дебаффы и эффекты. Юниты с Madness и Mechanism имеют 0 морали и не бросают её (кроме как под Break, когда эти способности отключены).",
                "Точные изменения:",
                bullet([
        ...
        },
        {
            ...
            text: [
                ...
                "Every drafted creature arrives as a stack worth 1,000–1,136 experience (its creature count is rounded up), so an unsplit stack normally starts the fight at stack power 5. Life's Supply synergy at level 3 (an all-Life army) grows a Monk stack past 1,250 (8 → 9, 1,278), and while all nine live, the opponent's stacks worth 1,022 or less — every exactly-1,000 stack among them — and your own level-4 stacks of 1–3 creatures (which don't grow) start at 4. Split a stack in half and both halves drop to 3 — 2 next to such a Supply stack — so a Stun that procs 35% at full stack procs 21%; a stack that has lost 60% of its creatures is down to 2. A split trades ability strength for board presence.",
            ].join("\n\n"),
            textRu: [
                ...
                "Каждое задрафтованное существо приходит стеком на 1000–1136 опыта (число существ округляется вверх), поэтому неразделённый стек обычно начинает бой с силой 5. Синергия Жизни «Запас» 3 уровня (армия целиком из Жизни) растит стек Monk больше 1250 (8 → 9, 1278), и пока живы все девять, стеки соперника на 1022 опыта и меньше — включая каждый стек ровно на 1000 — и ваши стеки 4 уровня из 1–3 существ (они не растут) начинают с 4. Разделите стек пополам — и обе половины опустятся до силы 3 (до 2 рядом с таким стеком «Запаса»), так что Stun, срабатывающий на 35% при полной силе, сработает на 21%; стек, потерявший 60% существ, опускается до 2. Разделение меняет силу способностей на присутствие на поле.",
            ].join("\n\n"),
        ...
        },
        {
            ...
            text: [
                "The Knowledge Base's ability cards print the full-stack value (stack power 5, luck 0); in the game many cards (Double Shot, Area Throw, Large Caliber, Chakram, Paralysis, Chain Lightning, the blessings…) already show the live value for the stack in front of you, so don't scale those again. A stack at stack power SP gets:",
                bullet([
                    "Trigger chances (Stun, Blindness, Dodge, Aggr, Hamstring, Terrifying Gaze, Spit Ball, Predatory Assimilation, Magic Reflection…): card value ÷ 5 × SP, plus 1 percentage point per point of luck, plus the Might ability synergy (+5/+8/+12; not for Magic Reflection). Paralysis rolls twice that; Petrifying Gaze turns a random ⅔ to all of that number into extra kills; its petrify roll is a quarter of it plus 4 per target level above 1 (at most 35%), ×0.75 per range band beyond the first — 15% against level 1 and 27% against level 4 at full stack.",
            ...
            textRu: [
                "Карточки способностей в Базе знаний показывают значение при полной силе стека (5) и нулевой удаче; в игре многие карточки (Double Shot, Area Throw, Large Caliber, Chakram, Paralysis, Chain Lightning, благословения…) уже показывают текущее значение для этого стека — не пересчитывайте их повторно. Стек с силой SP получает:",
                bullet([
                    "Шансы срабатывания (Stun, Blindness, Dodge, Aggr, Hamstring, Terrifying Gaze, Spit Ball, Predatory Assimilation, Magic Reflection…): значение ÷ 5 × SP плюс 1 процентный пункт за очко удачи плюс синергия Силы «Сила способностей» (+5/+8/+12; кроме Magic Reflection). Paralysis бросает удвоенный шанс; Petrifying Gaze превращает в дополнительные убийства случайные от ⅔ до всего этого числа; бросок окаменения — четверть его плюс 4 за каждый уровень цели выше первого (не больше 35%), ×0,75 за каждую полосу дальности после первой — 15% против 1 уровня и 27% против 4 уровня при полной силе.",
        ...
        },
        {
            ...
            text: [
                ...
                "In the first lap the army with the higher average morale starts (then the one with the higher top initiative, then a coin flip) . In every lap, stacks that rolled Morale act first and the alternation then starts with the army opposite the last of them; with no Morale roll, a lap starts with the army opposite the stack that acted last in the previous lap (a Dismorale stack or an Hourglass waiter included). Initiative therefore orders stacks only within their own army — a Morale roll is the only way to act ahead of the alternation. Each turn is one action: move (arriving ends the turn), melee attack (walking in first if needed), shoot, cast, Hourglass, Luck Shield or skip.",
            ].join("\n\n"),
            textRu: [
                ...
                "В первом круге начинает армия с более высокой средней моралью (затем — с большей максимальной инициативой, затем — жребий). В каждом круге стеки с сработавшей Morale ходят первыми, а очерёдность затем начинается с армии, противоположной последнему из них; без Morale круг начинается с армии, противоположной стеку, который ходил последним в прошлом круге (включая стек с Dismorale или ожидающий через Hourglass). Поэтому инициатива упорядочивает стеки только внутри своей армии — сработавшая Morale единственный способ походить раньше очереди. Каждый ход — одно действие: перемещение (прибытие завершает ход), ближняя атака (с подходом, если нужно), выстрел, заклинание, Hourglass, Luck Shield или пропуск.",
            ].join("\n\n"),
        ...
        },
        {
            ...
            text: [
                ...
                bullet([
                    "Timing: both hits are computed from the stacks as they stand, and the retaliation is applied first — you can't weaken a counterattack by hitting harder. The exception is an Area Throw, Large Caliber or Chakram shot: its whole volley, the target included, lands first, and only the survivors shoot back.",
                    "A melee blow goes unanswered when the defender is Stunned, Blinded or Frozen, has No Melee, is Aggr-locked onto another stack, is barred from this attacker by Terrifying Gaze, or is under Cowardice facing a stack with more total health. Shadow Touch and Lightning Spin attacks are never answered while the attacker isn't Broken. Size doesn't matter.",
                    "A shooter retaliating in melee deals half damage unless it has Handyman.",
                    "A shot is answered only by a shooter that could shoot right now: arrows left, no enemy touching it, no Range Null Field or Rangebane. Through Shot units never answer. The counter-shot flies back along its line, hits the first enemy stack on it, has its own falloff and Smoke, and costs an arrow; splash from Area Throw, Large Caliber and Chakram lands before it, and a shooter the splash wipes out doesn't answer.",
        ...
        },
        {
            ...
            text: bullet([
                ...
                "Area Throw, Large Caliber and Chakram apply the target's distance divisor to every unit they catch; a Through Shot gives each pierced unit its own distance and Smoke divisor. Large Caliber and Chakram damage also scales with stack power on every unit hit, the target included (a split stack at stack power 3 deals 60%), while Area Throw and Through Shot keep full power at any stack size.",
        ...
        },
        {
            ...
            text: bullet([
                "Magic resistance: magic damage is multiplied by (1 − resistance); a debuff spell — and the Spit Ball, Hamstring and Vine Throw debuffs — is resisted when a d100 roll lands under it (the scroll is spent either way). Creatures start with 0 / 5 / 8–12 / 15 by level; Armor augment points add to that base directly (15 + 21 = 36), and Magic Shield, Wardguard and the Warding Mane and Arcane Ward blessings then add as separate rolls: 1 − (1 − own)(1 − bonus). Armor never reduces spell damage; Fire Breath, though, is rolled like a melee hit on each unit it reaches (its armor and luck count) before magic resistance, and Chain Lightning, Fire Shield and Fireforged burns are shares of hits that armor already reduced.",
                "100% magic resistance (Enchanted Skin): immune to every spell from either side — buffs, heals and mass spells skip it — and to ability magic damage (Chain Lightning, Fire Breath, Fire Shield, Fireforged burns, Fire Wall); Spit Ball, Hamstring, Vine Throw and Rime Charm debuffs always fail on it. Not immune to on-hit status effects (Stun, Blindness, Paralysis, Petrifying Gaze…), auras, blessings, Craft or Resurrection. Break lifts it for 2 laps.",
                "Status resistance (Amulet of Resolve, 25%): Stun, Freeze and Paralysis chances are multiplied by (1 − resistance), and so is physical area damage taken. Mechanism units count −50 here, so they take +50% from physical area attacks; Stun and Freeze are also 1.5× as likely to land on them, and Paralysis and Shatter Armor hit them 1.5× harder.",
            ...
            textRu: bullet([
                "Сопротивление магии: магический урон умножается на (1 − сопротивление); дебафф заклинания — а также дебаффы Spit Ball, Hamstring и Vine Throw — отражается, если бросок d100 меньше него (свиток тратится в любом случае). У существ оно изначально 0 / 5 / 8–12 / 15 по уровням; очки апгрейда «Броня» прибавляются к этой базе напрямую (15 + 21 = 36), а Magic Shield, Wardguard и благословения Warding Mane и Arcane Ward затем добавляются как отдельные броски: 1 − (1 − своё)(1 − бонус). Броня урон заклинаний не снижает; но Fire Breath считается как удар в ближнем бою по каждому задетому юниту (его броня и удача учитываются) до сопротивления магии, а Chain Lightning, Fire Shield и поджоги Fireforged — доли ударов, уже сниженных бронёй.",
                "100% сопротивления магии (Enchanted Skin): иммунитет ко всем заклинаниям обеих сторон — баффы, лечение и массовые заклинания его пропускают — и к магическому урону способностей (Chain Lightning, Fire Breath, Fire Shield, поджоги Fireforged, Fire Wall); дебаффы Spit Ball, Hamstring, Vine Throw и Rime Charm на него никогда не ложатся. Нет иммунитета к эффектам ударов (Stun, Blindness, Paralysis, Petrifying Gaze…), аурам, благословениям, Craft и Resurrection. Break снимает её на 2 круга.",
                "Сопротивление статусам (Amulet of Resolve, 25%): шансы Stun, Freeze и Paralysis умножаются на (1 − сопротивление), как и получаемый физический урон по площади. У Mechanism здесь −50, поэтому они получают +50% от физических атак по площади; кроме того, Stun и Freeze срабатывают на них в 1,5 раза чаще, а Paralysis и Shatter Armor действуют в 1,5 раза сильнее.",
        ...
        },
        {
            ...
            text: [
                ...
                "Per victim: Heavy Armor's magic multiplier first (up to ×1.5), then the element (its own element deals 0 and can't be targeted; the opposed element ×1.5), then × (1 − magic resistance). Armor and luck never count, a Water Shield absorbs the hit, and spells never cause Break. Stack power only decides whether the spell can be cast.",
        ...
        },
        {
            ...
            text: [
                DURATION_NOTE.en,
                "Board effects count lap changes instead: Smoke lasts up to 3 full laps (a creature that ends its move on smoked cells clears the smoke there), and Fire Wall and vines disappear at the third lap change after they were cast. Permanent changes (runes, Craft's second attack and frozen weapons, Crusade, Miner, Dulling Defense, Bitter Experience) never expire.",
                "A spell can't be cast on a stack that still carries it (the same buff or the same debuff), so recasting never refreshes or stacks a duration — only the Blacksmith's Armor and Weapon Runes stack. An active Stun, Blindness or Freeze isn't extended either.",
            ].join("\n\n"),
            textRu: [
                DURATION_NOTE.ru,
                "Эффекты на поле считают смены кругов: Smoke держится до 3 полных кругов (существо, закончившее перемещение на клетках с дымом, рассеивает его там), а Fire Wall и лоза исчезают на третьей смене круга после применения. Постоянные изменения (руны, вторая атака и замороженное оружие от Craft, Crusade, Miner, Dulling Defense, Bitter Experience) не истекают никогда.",
                "Заклинание нельзя наложить на стек, на котором оно ещё действует (тот же бафф или тот же дебафф), поэтому повторное применение никогда не обновляет и не складывает длительность — складываются только руны Blacksmith (Armor Rune и Weapon Rune). Действующие Stun, Blindness и Freeze тоже не продлеваются.",
            ].join("\n\n"),
        ...
        },
        {
            ...
            textRu: bullet([
                "Рейтинговая карта — Normal, Barrels или FIRE PIT, с равными шансами — определяется при создании драфта и видна обоим игрокам во время драфта. На Barrels ровно 12 разрушаемых бочек, расставленных по четырём центральным колонкам по ID матча; единого блока в центре нет.",
        ...
        },
        {
            ...
            summaryRu:
                "Какие способности, ауры, артефакты, апгрейды, заклинания и синергии усиливают друг друга, какие мешают — и числа за каждым сочетанием, проверенные в коде движка.",
            text: [
                "What works together and what works against itself — each interaction checked in the engine's code (common @ 9442995). Numbers are at full stack power unless stated.",
                "#### Deep Wounds, luck and many attackers\nEvery melee hit by a Deep Wounds holder adds (card value ÷ 5 × stack power) + luck + Might's Abilities power synergy to the target's wounds, and every attacker holding a Deep Wounds card deals the wound total as extra percent damage. Luck counts in full, so on the smaller cards it adds more than the card: a Wolf's 6 per hit becomes 16 at +10 luck (a Leprechaun's Luck Aura sets allies within 2 cells to exactly +10) and 24 with Abilities power level 2 as well (level 3 is an all-Might army, with no room for a Wolf); level-2 cards (Griffin, White Tiger, Manticore) go 13 → 23, the Behemoth's 21 → 31. A Wolf's Double Punch lays two wounds in one attack — the second punch is already priced with the wound the first one made. Wounding Charm gives every unit a level-1 card, so every melee hit adds wounds and every attacker — shooters too — cashes them in.",
                "#### Luck and auras\nPercentage auras add their carrier's luck and Might's Abilities power in full: at +10 luck and Abilities power level 2 — the most a Life carrier can have, since level 3 is an all-Might army — Sharpened Weapons (Crusader) goes 18 → 36% base attack for melee allies, War Anger (Valkyrie) 10 → 28% per enemy stack nearby, Absorb Penalties (Peasant) 50 → 68% (Guiding Winds and Sylvan Focus take the luck but not the synergy; Venom Cloud is a flat 20% plus the luck of the ally that lands the hit, so the Wyvern's own luck doesn't matter). A Leprechaun standing next to an aura carrier therefore strengthens the aura for every ally inside it — and its Luck Aura covers itself: its Lucky Strike goes from a 50% chance of +50% (about ×1.25 on average) to 58% for +58% with Abilities power level 2 (a Nature Leprechaun can't be in the all-Might army level 3 needs). Luck also makes the Blacksmith's Craft safe: its self-stun chance is 10% minus luck, so at +10 it never backfires and the Frozen outcome rises to 20%.",
                "#### Aura rules\nAuras of the same name never stack: every cell keeps the strongest source. Flat auras don't depend on stack power — Luck, Range Null Field, Web, Pegasus Might (+2), Tie up the Horses (+2), Venom Cloud (20%), Rallying Volley (+2 shots), Sylvan Focus, and the Angelic Host and Arcane Ward blessings — so a split carrier projects them at full strength in two places (its other stack-powered abilities still drop with the split). Stack-powered ones (Sharpened Weapons, War Anger, Absorb Penalties, Wolf Trail, Guiding Winds, Flesh Shield; the Warding Mane and Arrows Wingshield blessings) weaken when split, so keep those carriers whole. Might's Aura Range synergy adds 1/2/3 cells to every aura, enemy-facing ones included: a 2×2 carrier's 6×6 area grows to 10×10 — 100 of the board's 256 cells — at level 2, the most a Griffin (Life) or an Arachna Queen (Nature) can get, which turns its Range Null Field or Web into a much bigger threat (level 3's 12×12 needs an all-Might army); it also widens the White Tiger's Disguise detection area (8×8 → 12×12 at level 2), so the Tiger is visible far more often. Flesh Shield pays most on fragile allies: the share it takes is re-priced against the Abomination's 45 armor, so it saves a low-armor shooter far more than a tank.",
                "#### Locks and retaliation\nUnicorn: One in the Field answers every melee attack, not just the first each lap, and every answer rolls Blindness — 25% at full stack, 33% with Abilities power level 2 (the most a Nature Unicorn can get), 43% with +10 luck as well; a blinded attacker skips 2 turns and loses its Double Punch follow-up. Attack it with shooters, spells or Mind-immune units (Madness, Mechanism); Helm of Focus cuts the roll to ×0.65. Pikeman: Aggr (50% at full stack) makes the target answer only the Pikeman for a lap, so every other ally hits it unanswered, and Skewer Strike rolls the whole on-hit package on the unit behind a single-cell target too — one attack can lock two stacks; act with the Pikeman first. Stun, Blindness and Freeze don't refresh while active and different ones count down together, so a Stun on a stack that is already Blinded is wasted — spread disables across targets. A retaliation lands before a Double Punch's second punch, so a Stun rolled by the answering Squire or Orc cancels that punch and a Mantis's Paralysis cuts it by 40%. Misfortune (Wandering Mage) drops a stack's luck to −10, but a stack under a Leprechaun's Luck Aura or Clover of Fortune ends at 0 instead.",
                "#### Breaking armor\nDamage divides by armor, so armor cuts multiply. Piercing Spear ignores (50 + luck + Abilities power)% of whatever armor is left after Shatter Armor (Nomad, −5 per attack, stacking) and other cuts: a full-stack Centaur deals ×2 damage against armor, ×2.5 at +10 luck, ×3.57 with Abilities power level 3 as well, and its Boost Health uses the same formula (+50% → +72% health). Armor never drops below 1. Centaur armies gain more from luck and Abilities power than any other; send the Nomad in first against high-armor targets.",
                "#### Movement is damage\nRapid Charge adds its per-cell percentage for every cell of the charge, the starting cell included: 10% per cell at full stack, 20% at +10 luck, 32% with Abilities power level 3 too (an all-Might army: Wolf Rider or Nomad; a Life Champion tops out at level 2, 28%) — walking 6 cells counts 7, so ×1.7, ×2.4 or ×3.24. Attacks only, not retaliations. Everything that lengthens the route — the Movement augment, Swift Boots (+25%, applied after the augment), Chaos Movement, Crown of Command, Wolf Trail, Tie up the Horses, Battle Roar — is a damage bonus for the Wolf Rider, Nomad and Champion, and the Champion's Crusade adds a permanent +0.5 attack and armor per cell walked (up to 50).",
                "#### Artifacts that reward a build\n- Hunter's Longbow: +1 base attack per ranged stack, added after the Sniper augment's percentage — with 5 ranged stacks an Arbalester goes 7 → 12 (+71%), a Tsar Cannon only 46 → 51 (+11%). It is an artifact for swarms of cheap shooters.\n- Farsight Quiver, Guiding Winds and the Sniper augment's range do nothing for the Arbalester (its Sniper ability already ignores falloff); they matter for short-range shooters — a Centaur's 4 cells become 6 with Farsight Quiver and 8 with Sniper level 3 as well.\n- Tome of Amplification strengthens every numbered buff your units cast on your own side: Riot 30 → 45%, Mass Riot 25 → 37.5%, Magic Mirror 40 → 60%, Mass Magic Mirror 32 → 48%, Battle Roar +1 → +1.5 movement per Behemoth — and Wind Flow, which then gives your own flyers +6 armor but −6 movement (enemy flyers keep ±4): cast it only once your flyers are in position.\n- Fireforged Sword adds 20% of the damage that landed as fire on melee and ranged attacks alike (30% with Tome). Area attacks burn every creature that took damage, each for its own share: a Pikeman's Skewer Strike, a Gargantuan's Area Throw, a Tsar Cannon's Through Shot, a Hydra's Lightning Spin and a Zena's Chakram. Magic-damage bonuses do not raise that share.\n- Rallying Volley (Zena) gives each ranged stack within 2 cells +2 shots once: Tsar Cannon and Centaur 4 → 6, Orc 6 → 8; every split stack gets its own +2.\n- Pegasus Might's flat +2 attack and armor comes before the percentage bonuses, so it matters most on level-1 stacks: a Peasant's 9/7 becomes 11/9 (+22%/+29%), a Black Dragon's 48/40 barely moves.\n- Giant's Maul multiplies physical area damage on every unit hit — your own stacks in the ring of your Area Throw or Large Caliber included (a Tsar Cannon there takes ×1.4 × 1.5 = ×2.1), so keep them out of the 3×3 around your own targets.\n- Amulet of Resolve's 25% status resistance also cuts physical area and line damage, so an enemy Giant's Maul (×1.4) nets only ×1.05 against it.",
            ...
            textRu: [
                ...
                "#### Deep Wounds, удача и много атакующих\nКаждый удар в ближнем бою юнита с Deep Wounds добавляет к ранам цели (значение карты ÷ 5 × сила стека) + удача + синергия Силы «Сила способностей», а каждый атакующий с картой Deep Wounds наносит сумму ран как дополнительный урон в процентах. Удача входит целиком, поэтому на маленьких картах она даёт больше самой карты: 6 за удар у Wolf становятся 16 при удаче +10 (Luck Aura у Leprechaun ставит союзникам в радиусе 2 клеток ровно +10) и 24 вместе с «Силой способностей» 2 уровня (3 уровень — это армия целиком из Силы, где Wolf нет места); карты 2 уровня (Griffin, White Tiger, Manticore) — 13 → 23, у Behemoth — 21 → 31. Double Punch у Wolf кладёт две раны за одну атаку: второй удар уже считается с раной, которую нанёс первый. Wounding Charm даёт каждому юниту карту 1 уровня: каждый удар в ближнем бою добавляет раны, а пользуется ими каждый атакующий — и стрелки тоже.",
                "#### Удача и ауры\nПроцентные ауры прибавляют удачу своего носителя и синергию «Сила способностей» целиком: при удаче +10 и «Силе способностей» 2 уровня — больше носителю из Жизни не получить, ведь 3 уровень — это армия целиком из Силы, — Sharpened Weapons (Crusader) растёт с 18 до 36% базовой атаки союзников ближнего боя, War Anger (Valkyrie) — с 10 до 28% за каждый вражеский стек рядом, Absorb Penalties (Peasant) — с 50 до 68% (Guiding Winds и Sylvan Focus берут удачу, но не синергию; Venom Cloud — это ровно 20% плюс удача союзника, наносящего удар, так что удача самой Wyvern не важна). Поэтому Leprechaun рядом с носителем ауры усиливает ауру для всех союзников в ней — а его Luck Aura действует и на него самого: Lucky Strike идёт от 50% шанса на +50% (в среднем около ×1,25) до 58% на +58% с «Силой способностей» 2 уровня (Leprechaun из Природы не попадёт в армию целиком из Силы, которой требует 3 уровень). Удача делает безопасным и Craft у Blacksmith: шанс оглушить себя — 10% минус удача, так что при +10 он не срабатывает против вас, а шанс Frozen растёт до 20%.",
                "#### Правила аур\nАуры с одинаковым названием не складываются: каждая клетка берёт сильнейший источник. Плоские ауры не зависят от силы стека — Luck, Range Null Field, Web, Pegasus Might (+2), Tie up the Horses (+2), Venom Cloud (20%), Rallying Volley (+2 выстрела), Sylvan Focus, благословения Angelic Host и Arcane Ward, — поэтому разделённый носитель даёт их в полную силу в двух местах (остальные его способности от силы стека после разделения слабеют). Ауры от силы стека (Sharpened Weapons, War Anger, Absorb Penalties, Wolf Trail, Guiding Winds, Flesh Shield; благословения Warding Mane и Arrows Wingshield) при разделении слабеют — таких носителей держите целыми. Синергия Силы «Радиус аур» добавляет 1/2/3 клетки каждой ауре, в том числе направленной на врага: зона носителя 2×2 растёт с 6×6 до 10×10 — 100 из 256 клеток поля — на 2 уровне, а больше Griffin (Жизнь) или Arachna Queen (Природа) не получить, и их Range Null Field или Web становятся куда опаснее (12×12 на 3 уровне требует армии целиком из Силы); заодно растёт зона обнаружения Disguise у White Tiger (8×8 → 12×12 на 2 уровне), и Tiger куда чаще видим. Flesh Shield выгоднее всего на хрупких союзниках: забранная доля пересчитывается против брони Abomination (45), поэтому стрелка с низкой бронёй он спасает куда сильнее, чем танка.",
                "#### Контроль и ответы\nUnicorn: One in the Field отвечает на каждую атаку в ближнем бою, а не только на первую за круг, и каждый ответ бросает Blindness — 25% при полной силе, 33% с «Силой способностей» 2 уровня (больше Unicorn из Природы не получить), 43% ещё и с удачей +10; ослеплённый атакующий пропускает 2 хода и теряет второй удар Double Punch. Бейте его стрелками, заклинаниями или юнитами с иммунитетом к Разуму (Madness, Mechanism); Helm of Focus умножает шанс на 0,65. Pikeman: Aggr (50% при полной силе) заставляет цель отвечать на круг только ему, поэтому все остальные союзники бьют её без ответа, а Skewer Strike бросает весь набор эффектов удара и по юниту позади цели размером в одну клетку — одна атака может связать два стека; ходите Pikeman первым. Stun, Blindness и Freeze не обновляются, пока действуют, а разные эффекты убывают одновременно, поэтому Stun на уже ослеплённом стеке пропадает зря — распределяйте контроль по разным целям. Ответ наносится до второго удара Double Punch, поэтому Stun от отвечающего Squire или Orc отменяет этот удар, а Paralysis от Mantis ослабляет его на 40%. Misfortune (Wandering Mage) опускает удачу стека до −10, но стек под Luck Aura от Leprechaun или с Clover of Fortune получает 0.",
                "#### Пробитие брони\nУрон делится на броню, поэтому снижения брони перемножаются. Piercing Spear игнорирует (50 + удача + «Сила способностей»)% брони, оставшейся после Shatter Armor (Nomad, −5 за атаку, складывается) и других снижений: Centaur при полной силе наносит ×2 урона, при удаче +10 — ×2,5, а вместе с «Силой способностей» 3 уровня — ×3,57; его Boost Health считается так же (+50% → +72% здоровья). Броня не опускается ниже 1. Армии с Centaur получают от удачи и «Силы способностей» больше всех; против бронированных целей пускайте Nomad первым.",
                "#### Движение — это урон\nRapid Charge добавляет свой процент за каждую клетку разбега, считая клетку старта: 10% за клетку при полной силе, 20% при удаче +10, 32% ещё и с «Силой способностей» 3 уровня (армия целиком из Силы: Wolf Rider или Nomad; Champion из Жизни получает максимум 2 уровень — 28%) — 6 пройденных клеток считаются как 7, то есть ×1,7, ×2,4 или ×3,24. Только в атаке, не в ответах. Всё, что удлиняет путь, — апгрейд «Движение», Swift Boots (+25%, после апгрейда), синергия Хаоса «Передвижение», Crown of Command, Wolf Trail, Tie up the Horses, Battle Roar — это бонус к урону для Wolf Rider, Nomad и Champion, а Crusade у Champion навсегда добавляет +0,5 к атаке и броне за каждую пройденную клетку (до 50).",
                "#### Артефакты под сборку\n- Hunter's Longbow: +1 к базовой атаке за каждый стек стрелков, прибавляется после процента апгрейда «Стрельба» — при 5 стеках стрелков Arbalester растёт с 7 до 12 (+71%), а Tsar Cannon — лишь с 46 до 51 (+11%). Это артефакт для стай дешёвых стрелков.\n- Farsight Quiver, Guiding Winds и дальность апгрейда «Стрельба» ничего не дают Arbalester (его Sniper и так отменяет штраф за дальность); они важны стрелкам с короткой дистанцией — 4 клетки Centaur становятся 6 с Farsight Quiver и 8 ещё и со «Стрельбой» 3 уровня.\n- Tome of Amplification усиливает каждый числовой бафф, который ваши юниты накладывают на свою сторону: Riot 30 → 45%, Mass Riot 25 → 37,5%, Magic Mirror 40 → 60%, Mass Magic Mirror 32 → 48%, Battle Roar +1 → +1,5 к движению за каждого Behemoth — и Wind Flow, который тогда даёт вашим летающим +6 к броне, но −6 к движению (вражеские остаются при ±4): применяйте его, только когда ваши летающие уже на месте.\n- Fireforged Sword добавляет 20% нанесённого урона огнём и в ближнем, и в дальнем бою (30% с Tome). Удар по площади поджигает каждое задетое существо на его собственную долю: Skewer Strike у Pikeman, Area Throw у Gargantuan, Through Shot у Tsar Cannon, Lightning Spin у Hydra и Chakram у Zena. Бонусы к магическому урону эту долю не повышают.\n- Rallying Volley (Zena) даёт каждому стеку стрелков в радиусе 2 клеток +2 выстрела один раз: Tsar Cannon и Centaur 4 → 6, Orc 6 → 8; каждый разделённый стек получает свои +2.\n- Плоские +2 к атаке и броне от Pegasus Might прибавляются до процентных бонусов, поэтому важнее всего на стеках 1 уровня: 9/7 у Peasant становятся 11/9 (+22%/+29%), а 48/40 у Black Dragon почти не меняются.\n- Giant's Maul умножает физический урон по площади по каждому поражённому — включая ваши стеки в кольце вашего Area Throw или Large Caliber (Tsar Cannon там получает ×1,4 × 1,5 = ×2,1), поэтому держите их вне 3×3 вокруг своих целей.\n- 25% сопротивления статусу от Amulet of Resolve режут и физический урон по площади и по линии, поэтому вражеский Giant's Maul (×1,4) даёт против него лишь ×1,05.",
        ...
        },
        {
            ...
            text: bullet([
                ...
                `Armageddon hits every stack of both armies at the start of laps ${NUMBER_OF_LAPS_FIRST_ARMAGEDDON}–${lastWave}, ignoring armor, luck and magic resistance (a Water Shield absorbs one wave). Counting every creature the stack started with, alive or dead: wave 1 deals 25% of their health (at least ${MIN_ARMAGEDDON_DAMAGE_FIRST_WAVE}), waves 2, 3 and 4 kill 50%, 75% and 100% of those creatures, rounded up to whole creatures — so a one-creature stack loses its only creature to wave 2.`,
                `The first three waves add up to 150%, so a stack that faces them all dies to wave 3 unless Resurrection returned creatures (a Water Shield alone can't save it: it only ever takes wave 1, and waves 2 and 3 together are 125%); wave 4 destroys everything left, so a fight still running at lap ${lastWave} is a draw. Armies that lose their last stacks to the same wave draw. Waves 2–4 remove whole creatures (that share of the starting count, rounded up — a lone Black Dragon loses its one creature, all 300 health, to wave 2), so one-creature stacks (Black Dragon, Abomination, any one-creature split) and any stack at three-quarters of its starting health or less already die to wave 2.`,
            ]),
            textRu: bullet([
                ...
                `Армагеддон бьёт по каждому стеку обеих армий в начале ${NUMBER_OF_LAPS_FIRST_ARMAGEDDON}–${lastWave}-го кругов без учёта брони, удачи и сопротивления магии (Water Shield поглощает одну волну). Если считать всех существ, с которыми стек начинал, живых и погибших: первая волна наносит 25% их здоровья (не меньше ${MIN_ARMAGEDDON_DAMAGE_FIRST_WAVE}), вторая, третья и четвёртая убивают 50%, 75% и 100% этих существ, с округлением вверх до целых — поэтому стек из одного существа теряет его во второй волне.`,
                `Первые три волны вместе — это 150%, поэтому стек, встретивший их все, гибнет от третьей, если Resurrection не вернул существ (один Water Shield его не спасёт: он принимает только первую волну, а вторая и третья вместе — уже 125%); четвёртая уничтожает всех оставшихся, так что бой, доживший до ${lastWave}-го круга, — ничья. Армии, потерявшие последние стеки от одной волны, играют вничью. Волны 2–4 убирают существ целиком (эту долю начального числа, с округлением вверх — одиночный Black Dragon теряет своё единственное существо, все 300 здоровья, во второй волне), поэтому стеки из одного существа (Black Dragon, Abomination, любая одиночная часть разделения) и любой стек с тремя четвертями начального здоровья или меньше гибнут уже от второй волны.`,
            ]),
        ...
        },
        {
            ...
            text: bullet([
                `Each army has ${minutes} minutes per lap. A turn lasts min(${maxTurn} s, remaining budget ÷ its living stacks still to act), always leaving ${minTurn} s for each of them — an army with 8 stacks alive gets about 30 s per turn at the start of a lap, one with 4 or fewer ${maxTurn} s. Losses lengthen the turns of the stacks left, and summoned stacks share the budget too. Time spent before an Hourglass wait isn't charged.`,
            ...
            textRu: bullet([
                `У каждой армии ${minutes} минуты на круг. Ход длится min(${maxTurn} с, остаток бюджета ÷ её живые стеки, которые ещё не ходили), и каждому из них всегда остаётся ${minTurn} с — армия с 8 живыми стеками в начале круга получает около 30 с на ход, с 4 и меньше — ${maxTurn} с. Потери удлиняют ходы оставшихся стеков, а призванные стеки тоже делят бюджет. Время до ожидания через Hourglass не списывается.`,
        ...
        },
        {
            ...
            text: bullet([
                ...
                `The stack cap is 6, raised to 7 or ${MAX_UNITS_PER_TEAM} by the Placement augment; Nature's Board Units synergy adds another 2/3/4, for an absolute cap of 12. Splitting a unit into more stacks creates no creatures, arrows or spell charges: the parts share the stack's arrows and charges (the new part's share is rounded down, so a single charge such as Resurrection stays with the original), and merging a part back returns them.`,
                ...
                `Faction synergies reach level 1/2/3 at ${unitsForLevel(1)}/${unitsForLevel(2)}/${unitsForLevel(3)} distinct roster units of that faction (3 still give level 1); splitting a unit into several stacks does not raise the count. Once the fight starts the level never drops as stacks die, and a summoned creature the army didn't field (the Wolf from Summon Wolves) counts as one more.`,
            ]),
            textRu: bullet([
                ...
                `Лимит стеков — 6; апгрейд «Расстановка» поднимает его до 7 или ${MAX_UNITS_PER_TEAM}; синергия Природы «Отряды на поле» добавляет ещё 2/3/4, абсолютный максимум — 12. Разделение юнита на стеки не создаёт ни существ, ни стрел, ни зарядов заклинаний: части делят стрелы и заряды стека (доля новой части округляется вниз, поэтому единственный заряд — например, Resurrection — остаётся у исходного стека), а при обратном объединении они возвращаются.`,
...
export function buildKnowledgeGraph(options: BuildKnowledgeGraphOptions = {}): KnowledgeGraph {
    ...
    const unitPluralAliases: Record<string, string[]> = {
        Elf: ["Elves"],
        Wolf: ["Wolves"],
        Pegasus: ["Pegasi"],
        Fairy: ["Fairies"],
        ...
        graph.add({
            ...
            text: [
                ...
                bullet(
                    levels.map(
                        (entry) =>
                            `Level ${entry.level} (${entry.units} distinct ${spec.faction} units${entry.level === 3 ? ` — the whole draft, so only ${spec.faction} units are in the army` : ""}): ${spec.effect(entry.powers)}`,
                    ),
                ),
                "Splitting one unit into several stacks does not raise the unit count.",
                `The bonus applies to every unit in your army, whatever its faction. The level is the number of different creatures of the faction ÷ 2, rounded down (3 creatures still give level 1), locked when the fight starts — losses never lower it. Only Nature has summons (Wolves, Arachna Spider or Queen): one new to your army can raise Nature's level mid-fight. A draft brings exactly six creatures (2/2/1/1), so level 3 means every drafted creature belongs to this faction.`,
            ...
            textRu: [
                ...
                bullet(
                    levels.map(
                        (entry) =>
                            `Уровень ${entry.level} (${entry.units} разных юнитов фракции${entry.level === 3 ? " — весь драфт, так что в армии только юниты этой фракции" : ""}): ${spec.effectRu(entry.powers)}`,
                    ),
                ),
                "Разделение одного юнита на несколько стеков счёт юнитов не увеличивает.",
                "Бонус действует на каждого юнита вашей армии, какой бы фракции он ни был. Уровень — число разных существ фракции ÷ 2 с округлением вниз (3 существа — всё ещё уровень 1); он фиксируется в начале боя, потери его не снижают. Призывы есть только у Природы (Wolf, Arachna Spider или Queen): новое для армии призванное существо может поднять уровень Природы посреди боя. Драфт даёт ровно шесть существ (2/2/1/1), поэтому 3 уровень означает, что все задрафтованные существа — этой фракции.",
