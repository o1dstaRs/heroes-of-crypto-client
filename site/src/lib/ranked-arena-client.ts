...
const renderGames = (
    ...
    if (remaining > 0) {
        const increment = Math.min(GAMES_PAGE_SIZE, remaining);
        const more = el(
            "button",
...
const initArena = (root: HTMLElement, heroLeaderboard: HeroLeaderboardController | null): void => {
    ...
    search.addEventListener("input", () => {
        ...
        state.visibleGames = GAMES_PAGE_SIZE;
        render();
    });
    ...
    filter.addEventListener("change", () => {
        ...
        if (state.tab === "games") state.visibleGames = GAMES_PAGE_SIZE;
        render();
    });
