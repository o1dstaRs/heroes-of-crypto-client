...
export const ABILITY_NOTES_M_Z: Readonly<Record<string, NoteSpec>> = {
    ...
    "Magic Reflection": {
        en: ({ p, n }) =>
            `Each hostile spell that lands on the dragon has ${n(p / 5)}% per stack power (${n(p)}% at full stack) plus 1% per point of luck to rebound. The dragon still takes the whole spell. A rebounded damage spell hits the caster for that same percentage of the damage the dragon took, reduced by the caster's own element and magic resistance; a rebounded debuff (a spell, Spit Ball or Hamstring) lands on the caster as well. Magic damage from abilities — Fire Breath, Chain Lightning, Fire Shield, Fireforged Sword — isn't reflected. (The game card says the caster takes the same damage; the engine returns only that percentage of it.) A Magic Mirror on the dragon doesn't add to it: a damage spell returns Reflection's share when it procs and the Mirror's otherwise, a debuff is copied at the higher of the two chances, and only the Mirror returns ability magic damage.`,
        ru: ({ p, n }) =>
            `Каждое враждебное заклинание, попавшее в дракона, с шансом ${n(p / 5)}% за единицу силы стека (${n(p)}% при полной силе) плюс 1% за очко удачи отражается. Дракон всё равно получает заклинание целиком. Отражённое заклинание урона бьёт заклинателя на тот же процент от урона, полученного драконом, с учётом стихии и сопротивления магии самого заклинателя; отражённый дебафф (заклинание, Spit Ball или Hamstring) ложится и на заклинателя. Магический урон от способностей — Fire Breath, Chain Lightning, Fire Shield, Fireforged Sword — не отражается. (Карточка в игре обещает заклинателю тот же урон, а движок возвращает только этот процент.) Magic Mirror на драконе с ней не складывается: заклинание урона возвращается по доле Reflection, если она сработала, иначе по доле Mirror, дебафф копируется с большим из двух шансов, а магический урон способностей возвращает только Mirror.`,
    },
    "Magic Shield": {
        en: ({ p, n }) =>
    ...
    Resurrection: {
        en: () =>
            "One use per fight, whichever comes first. (1) When the Angel stack is wiped out in combat, by Fire Wall or by the first Armageddon wave, half the fallen Angels (rounded down, at least 1) return at full health with all effects removed — not while Broken, and not after a death to poison, narrowing or a later Armageddon wave. (2) Cast on any allied stack with losses, itself included, it restores health worth 1.5× the Angel stack's total max health (×1.25 on Morale, ×0.8 on Dismorale): the wounded creature first, then the dead, never more than died. Casting it gives up the self-resurrection. A stack that comes back isn't wiped out, so an enemy Infest gets nothing from it.",
        ru: () =>
