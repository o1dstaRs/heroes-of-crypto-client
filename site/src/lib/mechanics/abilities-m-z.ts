...
export const ABILITY_NOTES_M_Z: Readonly<Record<string, NoteSpec>> = {
    ...
    "Magic Reflection": {
        en: ({ p, n }) =>
            `Each hostile spell that lands on the dragon has ${n(p / 5)}% per stack power (${n(p)}% at full stack) plus 1% per point of luck to rebound. The dragon still takes the whole spell. A rebounded damage spell hits the caster for that same percentage of the damage the dragon took, reduced by the caster's own element and magic resistance; a rebounded debuff (a spell, Spit Ball or Hamstring) lands on the caster as well. Magic damage from abilities — Fire Breath, Chain Lightning, Fire Shield, Fireforged Sword — isn't reflected. (The game card says the caster takes the same damage; the engine returns only that percentage of it.)`,
        ru: ({ p, n }) =>
            `Каждое враждебное заклинание, попавшее в дракона, с шансом ${n(p / 5)}% за единицу силы стека (${n(p)}% при полной силе) плюс 1% за очко удачи отражается. Дракон всё равно получает заклинание целиком. Отражённое заклинание урона бьёт заклинателя на тот же процент от урона, полученного драконом, с учётом стихии и сопротивления магии самого заклинателя; отражённый дебафф (заклинание, Spit Ball или Hamstring) ложится и на заклинателя. Магический урон от способностей — Fire Breath, Chain Lightning, Fire Shield, Fireforged Sword — не отражается. (Карточка в игре обещает заклинателю тот же урон, а движок возвращает только этот процент.)`,
    },
    "Magic Shield": {
        en: ({ p, n }) =>
    ...
    Mechanism: {
        en: () =>
            "Immune to Mind abilities and spells and to poison; morale is always 0, so it never gets Morale or Dismorale. It can't be healed, though Resurrection still works on it. Status effects hit it harder: Stun and Freeze chances ×1.5, Paralysis and Shatter Armor 50% stronger. It takes 50% more damage from physical area and line attacks — Area Throw, Large Caliber, Chakram, Lightning Spin, Skewer Strike, Through Shot — which status resistance offsets point for point. There is no vampirism in the game, whatever the card says.",
        ru: () =>
            "Иммунитет к ментальным способностям, заклинаниям и яду; мораль всегда 0, поэтому Morale и Dismorale не срабатывают. Лечить его нельзя, но Resurrection на него действует. Эффекты Статуса бьют сильнее: шанс Stun и Freeze ×1,5, Paralysis и Shatter Armor на 50% сильнее. Получает на 50% больше урона от физических атак по площади и по линии — Area Throw, Large Caliber, Chakram, Lightning Spin, Skewer Strike, Through Shot, — а сопротивление статусам компенсирует это пункт в пункт. Вампиризма в игре нет, что бы ни писала карточка.",
    },
    Miner: {
        en: ({ p, n }) =>
    ...
    "One in the Field": {
        en: () =>
            "Retaliates against every melee attack it receives, with no once-per-lap limit. The usual blockers (Stun, Blindness, Freeze, Shadow Touch, Terrifying Gaze) still stop a retaliation, and it never answers shots.",
        ru: () =>
            "Отвечает на каждую атаку в ближнем бою, без ограничения раз за круг. Обычные запреты (Stun, Blindness, Freeze, Shadow Touch, Terrifying Gaze) по-прежнему мешают ответу, а на выстрелы он не отвечает.",
    },
    Paralysis: {
        en: ({ p, n }) =>
    ...
    "Small Specie": {
        en: ({ p, n }) =>
            `Physical attacks from creatures bigger than one cell miss this unit ${n(p / 5)}% of the time per stack power (${n(p)}% at full stack; ±1% per point of the Fairy's luck) — melee, retaliation, shots and every splash or pierce hit. It rolls separately from other dodge chances.`,
        ru: ({ p, n }) =>
            `Физические атаки существ крупнее одной клетки промахиваются по этому юниту в ${n(p / 5)}% случаев за единицу силы стека (${n(p)}% при полной силе; ±1% за очко удачи Fairy) — ближний бой, ответы, выстрелы и каждый удар по площади или насквозь. Бросок отдельный от других шансов уклонения.`,
    },
    Sniper: {
        en: () =>
