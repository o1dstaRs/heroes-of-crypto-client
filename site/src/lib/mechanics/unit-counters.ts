...
export function unitCounterLines(unit: Unit, language: NoteLanguage): string[] {
    ...
    if (unit.rangeShots > 0 && unit.attackType === "RANGE") {
        ...
        const rangebane = mindImmune ? "" : isRu ? ", а Rangebane — на круг" : ", and Rangebane for a lap";
        ...
        lines.push(
            isRu
                ? `Стрелок: враг в любой соседней клетке не даёт ему стрелять; Range Null Field Aura (Griffin) запрещает ему стрелять и отвечать выстрелом в радиусе 2 клеток${rangebane}; Smoke вдвое ослабляет его выстрелы${has("Sniper") ? " (и со Sniper)" : ""}; Arrows Wingshield Blessing (Angel) даёт армии соперника броню против выстрелов${denseFlesh}.${melee}${helpers}`
                : `Shooter: an enemy in any touching cell stops it shooting; Range Null Field Aura (Griffin) stops it shooting or shooting back within 2 cells${rangebane}; Smoke halves its shots${has("Sniper") ? " (Sniper shots too)" : ""}; Arrows Wingshield Blessing (Angel) armors the enemy army against shots${denseFlesh}.${melee}${helpers}`,
        );
    }
    ...
    if (unit.movementType === "FLY") {
        const slows = spellImmune
            ...
            : isRu
              ? "; Hamstring (Dryad) может отнять 30% движения на 3 круга; Wind Flow (Valkyrie) даёт всем летающим +4 к броне и −4 к движению на 3 круга"
              : "; Hamstring (Dryad) can take 30% of its movement for 3 laps; Wind Flow (Valkyrie) gives every flyer +4 armor and −4 movement for 3 laps";
        lines.push(
            isRu
    ...
    if (spells.length) {
        ...
        const resistance = isRu
            ? damage && debuffs
                ? ["сопротивление магии снижает урон его заклинаний и может противостоять его дебаффам, а Magic Mirror — скопировать их обратно на него"]
                : damage
                  ? ["сопротивление магии снижает урон его заклинаний"]
                  : debuffs
                    ? ["сопротивление магии может противостоять его дебаффам, а Magic Mirror — скопировать их обратно на него"]
                    : []
            : damage && debuffs
              ? ["magic resistance cuts its spell damage and can resist its debuffs, and Magic Mirror may copy them back onto it"]
              : damage
                ? ["magic resistance cuts its spell damage"]
                : debuffs
    ...
    if (has("Disguise Aura")) {
        lines.push(
            isRu
                ? "Disguise Aura: пока юнит скрыт, его нельзя выбрать целью; Break отключает её на 2 круга, а синергия Силы «Радиус аур» работает против неё — расширяет радиус, в котором враг раскрывает White Tiger (3 + 1/2/3 клетки)."
                : "Disguise Aura: while Hidden it can't be targeted; Break turns it off for 2 laps, and Might's Aura Range synergy works against it — it widens the radius in which an enemy reveals the White Tiger (3 + 1/2/3 cells).",
        );
    }
    ...
    const stackPowered = unit.abilities.filter((ability) => ability.isStackPowered && !NOT_SCALED_BY_STACK_POWER.test(ability.name)).map((ability) => ability.name);
